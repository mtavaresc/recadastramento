...
from migrate.paises import migrate_paises
from migrate.estados import migrate_estados
from migrate.municipios import migrate_municipios
from migrate.bairros import migrate_bairros
from migrate.tipos_logradouro import migrate_tipos_logradouro
from datetime import date
from _base import app
from _model import *

if Paises.query.count() == 0:
    migrate_paises(db)
if Estados.query.count() == 0:
    migrate_estados(db)
if Municipios.query.count() == 0:
    migrate_municipios(db)
if Bairros.query.count() == 0:
    migrate_bairros(db)
if TiposLogradouro.query.count() == 0:
    migrate_tipos_logradouro(db)
